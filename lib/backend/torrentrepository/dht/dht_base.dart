...
typedef NewPeerHandler = void Function(
    CompactAddress? address, String? hashinfo);
...
class DHT {
  ...
  void _processAnnouncePeerRequest(List<int>? idBytes, String? tid,
    ...
    CompactAddress? peer;
    dynamic impliedport = data['implied_port'];
  ...
  void _allFindNode(int index) {
    ...
    var nid = ID.createID(newId as List<int>);
    // print('bucket $index is all cleared, query the corresponding node ${nid.toString()}');
    _root?.forEach((node) {
      node?.queried = false;
  ...
  void _processGetPeersRequest(List<int>? idBytes, String? tid,
    ...
    var infoHashStr = String.fromCharCodes(infohash);
    var peers = _resourceTable[infoHashStr];
    // TO DO here to distinguish between IPv6 and IPv4!!!!!!!
  ...
  void _processGetPeersResponse(
    ...
    if (infoHash != null &&
      ...
      if (peerPort != null) {
        ...
        // print('Announcement Peer:Port $peerPort ,hash:$infoHash');
  ...
  /// Each node is 26 bytes, the first 20 are ID, the last 6 are IP and port
  void _processFindNodeResponse(
    ...
    if (node != null && node.queried) {
      // If the node is already in the local network and the node is found, the acquired nodes are no longer processed
      return;
    }
