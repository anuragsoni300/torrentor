...
import '../bencode_base.dart' as bencoding;
...
class Torrent {
  ///
  /// This is prepared to be able to regenerate the torrent file, because the parsed info will be ignored when generating the model
  /// , if you directly use the model to generate a torrent file, the original info information will be inconsistent, and you will not be able to get it if you parse it again.
  /// Correct sha1 information.
  ///
...
Future<T> _compution<T>(Function(dynamic) mainMethod, dynamic data) {
  ...
  Isolate.spawn<dynamic>(mainMethod, {'sender': port.sendPort, 'data': data},
...
Torrent? parseTorrentFileContent(Uint8List fileBytes) {
  ...
  if (announceList != null && announceList is Iterable) {
    if (announceList.isNotEmpty) {
      for (var urls in announceList) {
        // some are a set of arrays
        if (urls[0] != null && urls[0] is List) {
          urls.forEach((url) {
            try {
              var aurl = Uri.parse(_decodeString(url));
              torrentModel.addAnnounce(aurl); //if (aurl != null)
            } catch (e) {
              //
            }
          });
  ...
  if (torrent['url-list'] != null && torrent['url-list'] is Iterable) {
    ...
    for (var url in urlList) {
      try {
        var aurl = Uri.parse(_decodeString(url));
        torrentModel.addURL(aurl); //if (aurl != null)
      } catch (e) {
        //
      }
    }
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    torrentModel.addFile(TorrentFile(parts[parts.length - 1]!, p,
        file['length'], files.sublist(0, i).fold(0, _sumLength)));
  }
