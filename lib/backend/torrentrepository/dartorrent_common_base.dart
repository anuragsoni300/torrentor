...
class CompactAddress {
  ...
  static CompactAddress? parseIPv4Address(List<int>? message,
      [int offset = 0]) {
  ...
  static CompactAddress? parseIPv6Address(List<int>? message,
      [int offset = 0]) {
    if (message == null) return null;
...
Future<List<Uri>> _getTrackerFrom(String trackerUrlStr,
  ...
  try {
    ...
    client?.close();
    await Future.delayed(Duration(seconds: 15 * math.pow(2, retryTime) as int));
