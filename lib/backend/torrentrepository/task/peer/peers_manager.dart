...
const MAX_UPLOADED_NOTIFY_SIZE = 1024 * 1024 * 10; // 10mb

///
/// TO DO:
/// -No handling of external Suggest Piece/Fast Allow
class PeersManager with Holepunch, PEX {
  ...
  /// Cache maximum value written to disk
  ...
  /// What extensions are supported add here
  ...
  Future _flushFiles(final Set<int> indices) async {
    ...
    dynamic buffer = indices.length * piecesSize!;
  ...
  /// Even if the other party chokes me, I can download
  void _processAllowFast(dynamic source, int index) {
    var peer = source as Peer;
    dynamic piece = _pieceProvider?[index];
  ...
  void _pushSubpicesBack(List<List<int>?>? requests) {
    ...
    for (var element in requests) {
      ...
      var begin = element?[1];
      // TO DO It's very dangerous here. At present, it has been 16kb to decompose a piece, if not?
  ...
  void _requestPieces(dynamic source, [int pieceIndex = -1]) async {
    ...
    var size = DEFAULT_REQUEST_LENGTH; // Block size is calculated now
    ...
    if (bitfield != null) {
      ...
      if (_fileManager!.isAllComplete && peer.isSeeder) {
        peer.dispose(BadException(
            'The download has been completed and no longer connect to the seeder'));
        return;
      }
  ...
  void _processChokeChange(dynamic source, bool choke) {
    ...
    if (!choke) {
      ...
      // Start notification request here;
  ...
  void _processInterestedChange(dynamic source, bool interested) {
    ...
    if (interested) {
      ...
      peer.sendChoke(true); // choke it if you're not interested
    }
  }
  ...
  void _processRequestTimeout(dynamic source, List<List<int>?>? requests) {
    ...
    // wake up other peers that may not be working
    if (flag) {
  ...
  void pause() {
    ...
    _keepAliveTimer = Timer(const Duration(seconds: 110), _sendKeepAliveToAll);
  }
  ...
  void holePunchRendezvous(CompactAddress? ip) {
    // TO DO: implement holePunchRendezvous
