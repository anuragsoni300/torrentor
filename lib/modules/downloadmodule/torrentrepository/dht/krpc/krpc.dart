...
typedef KRPCQueryHandler = void Function(List<int>? nodeId,
    String? transactionId, InternetAddress? address, int? port, dynamic data);
...
class _KRPC implements KRPC {
  ...
  @override
  Future ping(InternetAddress? address, int? port) async {
    if (isStopped || _socket == null) return;
...
// print('There are currently $_totalPending requests');
  ...
  void _processQueryRequest(dynamic event) {
    ...
    // print('Send request $tid, the current number of pending requests: $_pendingQuery');
    _timeoutMap[tid!] =
  ...
  void _fireTimeout(String id) {
    ...
    if (event != null) {
      _reducePendingQuery();
      // print('The request timed out $id, the current number of pending requests: $_pendingQuery');
    }
  }
  ...
  void _processReceiveData(
    ...
    dynamic data;
    try {
      data = decode(Uint8List.fromList(bufferData!));
    } catch (e) {
    ...
    if (method == RESPONSE_KEY && data[RESPONSE_KEY] != null) {
      ...
      // Handle remotely sent responses
  ...
  void _getError(
    ...
    log('Get an error message from ${address?.address}:$port:',
  ...
  /// - `203`	Protocol Error, such as a malformed packet, invalid arguments, or bad token
  /// - `204`	Method Unknown
  void _fireError(int? code, String? tid, String? msg, InternetAddress? address,
      int? port) {
    if (tid != null) {
