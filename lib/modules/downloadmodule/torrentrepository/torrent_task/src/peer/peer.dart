...
import '../../../bencode_dart/bencode_dart.dart';
import '../../../dartorrent_common/dartorrent_common.dart';
import '../../../utp/utp.dart';
import '../../torrent_task.dart';
...
abstract class Peer
  ...
  void _processRemoteRequest(Uint8List? message) {
    if (_remoteRequestBuffer.length > reqq!.toInt()) {
  ...
  void _processReceivePieces(List<Uint8List> messages) {
    ...
    for (var message in messages) {
      ...
      _log('received a request Piece ($index,$begin) content, from the current Peer Downloaded $downloaded bytes ');
  ...
  Future? connectRemote(int timeout);
  ...
  List<int?>? _createByteMessage(int? id, List<int>? message) {
    ...
    length = length + 1;
    List<int?> datas = List<int>.filled(length + 4, 0);
...
class _TCPPeer extends Peer {
  ...
  @override
  Future connectRemote(int timeout) async {
    timeout = 30;
    try {
  ...
  void sendByteMessage(List<int?>? bytes) {
    try {
      if (bytes != null && bytes.isNotEmpty) _socket?.add(bytes as List<int>);
    } catch (e) {
      dispose(e);
    }
  }
