...
import '../../../torrent_model/torrent_model.dart';
...
class DownloadFileManager {
  ...
  DownloadFileManager(this.metainfo, this._stateFile) {
    _piece2fileMap = List.filled(_stateFile!.bitfield!.piecesNum, []);
  }
  ...
  int get downloaded => _stateFile?.downloaded ?? 0;
  ...
  void _initFileMap(String directory) {
    for (var i = 0; i < metainfo!.files.length; i++) {
      ...
      var df = DownloadFile(
        directory + file.path.replaceAll(String.fromCharCode(92), ''),
        file.offset,
        file.length,
      );
      _files.add(df);
