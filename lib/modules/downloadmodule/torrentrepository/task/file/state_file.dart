...
///
/// File content: `<bitfield><download>`, where `<download>` is a 64-bit integer,
/// Filename: `<infohash>.bt.state`
class StateFile {
  Bitfield? _bitfield;
  ...
  Future<File> init(String directoryPath, Torrent metainfo) async {
    ...
    // if (lastc != '\\' || lastc != '/') {
    directoryPath = '$directoryPath/${metainfo.name}/';
    // }

    _bitfieldFile = File('$directoryPath.${metainfo.infoHash}.bt.state');
    bool exists = await _bitfieldFile!.exists();
    if (!exists) {
  ...
  Future<dynamic> _update(event) async {
  ...
  Future<RandomAccessFile?> getAccess() async {
    if (_access == null) {
      ...
      _ss = _sc?.stream.listen(_processRequest, onError: (e) => log(e));
    }
    return _access;
  }
  ...
  Future<void> close() async {
    ...
    try {
      ...
      log('Error closing status file:', error: e, name: runtimeType.toString());
    } finally {
      _access = null;
      _ss = null;
      _sc = null;
