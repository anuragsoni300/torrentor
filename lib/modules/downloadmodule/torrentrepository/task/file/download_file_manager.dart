...
class DownloadFileManager {
  ...
  Map<String?, List<int>?> get file2pieceMap => _file2pieceMap;

  /// TO DO
  /// -No file read cache is established
  DownloadFileManager(this.metainfo, this._stateFile) {
    _piece2fileMap =
        List.filled(_stateFile!.bitfield!.piecesNum!, null, growable: true);
  }
  ...
  /// This method appears to only write the buffer contents to disk, but in fact
  /// Whenever the cache is written, it will consider that the [pieceIndex] corresponding to `Piece` has been completed, and it will be removed
  /// When the piece index corresponding to the file in `_file2pieceMap` is removed, a File Complete event will be thrown
  Future<bool> flushFiles(Set<int> pieceIndices) async {
    ...
    var msg =
        'Downloaded: ${d / (1024 * 1024)} mb , completeness ${((d / metainfo!.length!) * 10000).toInt() / 100} %';
  ...
  ///
  /// Write the contents of `Sub Piece` to a file. A `sub piece complete` event is sent when it is complete,
  /// If it fails, the `sub piece failed` event is sent
  ///
  /// The `Sub Piece` comes from the `Piece` corresponding to [pieceIndex], the content is [block], and the starting position is [begin].
  /// This class will not verify whether the written Sub Piece is repeated, and the repeated content directly overwrites the previous content
