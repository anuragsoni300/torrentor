...
/// The maximum request for each increase is 3
const MAX_CWND_INCREASE_REQUESTS_PER_RTT = 3 * 16384;

/// LEDBAT congestion control
///
/// Note that all time units are in microseconds
mixin CongestionControl {
  ...
  void updateRTO(int rtt) {
    ...
    // Less than 1 second, set to 1 second
  ...
  void ackRequest(List<List<int>?>? requests) {
    ...
    for (var request in requests) {
      // No matter what you receive after resending
      if (request == null || request[4] != 0) continue;
      var now = DateTime.now().microsecondsSinceEpoch;
