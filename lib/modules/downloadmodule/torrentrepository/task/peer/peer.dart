// ignore_for_file: constant_identifier_names, curly_braces_in_flow_control_structures, control_flow_in_finally
...
import '../../utp/utp_protocol_implement.dart';
import '../utils.dart';
...
/// Method with [index],[begin],[length] parameters
...
abstract class Peer
  ...
  /// The total number of pieces of the downloaded project
  ...
  /// Whether the other party has choke me, the initial default is true
  bool _chokeMe = true;

  /// Whether I choke the other party, the default is true
  bool chokeRemote = true;

  /// Whether the other party is interested in my resources, the default is false
  bool _interestedMe = false;

  /// Whether I am interested in the other party's resources, the default is false
  ...
  /// Local Peer Id
  final String?
      _localPeerId; // The peer id of this machine. Sending a message will use
  ...
  /// Remote data reception, monitoring subscription
  StreamSubscription? _streamChunk;

  /// Buffer to get data from channel
  List<int> _cacheBuffer = [];

  /// Send the request buffer locally. Format bits: [index,begin,length]
  final _requestBuffer = <List<int>>[];

  /// Send the request buffer remotely. Format bits: [index,begin,length]
  ...
  /// Allow Fast pieces sent remotely
  final Set<int> _remoteAllowFastPieces = <int>{};

  /// Suggest pieces sent remotely
  ...
  ///
  /// [_id] is the Id used to distinguish different Peers, which is different from [_localPeerId], [_localPeerId] is the Peer_id in the BitTorrent protocol.
  /// [address] is the address and port of the remote peer. Subclasses can use this value to connect remotely when implementing. [_infoHashBuffer]
  /// is the infohash value in the torrent file, [_piecesNum] is the total number of pieces of the downloaded item, used to build the remote `Bitfield` data
  /// use. The optional option [localEnableFastPeer] defaults to `true`, indicating whether the local enable [Fast Extension(BEP 0006)](http://www.bittorrent.org/beps/bep_0006.html),
  /// [localEnableExtended] indicates whether the [Extension Protocol] can be used locally (http://www.bittorrent.org/beps/bep_0010.html)
  ...
  factory Peer.newUTPPeer(String? localPeerId, CompactAddress? address,
      ...
      {bool enableExtend = true, bool enableFast = true}) {
    return _UTPPeer(
        localPeerId, address, infoHashBuffer, piecesNum, socket as UTPSocket?,
        enableExtend: enableExtend, enableFast: enableFast);
  }
  ...
  /// Whether the local bitfield has been sent to the other party
  bool get bitfieldSended => _bitfieldSended;

  bool get isLeecher => !isSeeder;

  /// If it has the complete torrent file, it is a seeder
  bool get isSeeder {
  ...
  /// request sent remotely
  List<List<int>> get remoteRequestbuffer => _remoteRequestBuffer;

  /// The request sent locally to the remote
  ...
  void _init() {
    ...
    // reset suggest pieces
    _remoteSuggestPieces.clear();
    // Reset the remote fast extension identity
  ...
  void _processReceiveData(dynamic data) {
    ...
    if (data != null)
      _cacheBuffer
          .addAll(data); // Accept remote to send data. buffer to one place
  ...
  /// delete the request from the requestbuffer
  ///
  /// This method is called whenever a piece response is received or the request times out
  ...
  void _processRejectRequest(Uint8List? message) {
    ...
    if (removeRequest(index, begin, length) != null) {
      ...
      // It may be deleted, but reject is slow
  ...
  void _processRemoteRequest(Uint8List? message) {
    ...
    if (chokeRemote) {
      if (_allowFastPieces.contains(index)) {
        ...
        // I don't need an answer to choke the other party
    ...
    _remoteRequestBuffer.add([index, begin, length]);
    // TO DO do speed limit here!
    fireRequest(index, begin, length);
  }

  /// Process received PIECE messages
  ///
  /// Unlike other message processing, PIECE messages are processed in batches.
  void _processReceivePieces(List<Uint8List>? messages) {
    ...
    messages?.forEach((message) {
      ...
      _log(
          'Received the content of the request Piece ($index,$begin), downloaded from the current Peer $downloaded bytes ');
  ...
  List<int> _createByteMessage(int id, List<int>? message) {
    ...
    length = length + 1;
    var datas = List<int>.filled(length + 4, 0, growable: true);
  ...
  ///
  /// After sending handshake, it will actively send bitfield and have messages to the other party
  void sendHandShake() {
  ...
  ///
  /// Delete if in the request queue, otherwise return
  ...
  void sendBitfield(Bitfield? bitfield) {
    _log('Send bitfile information to the other party: ${bitfield?.buffer}');
  ...
  void sendHave(int index) {
    ...
    _log('Send have information to the other party: $bytes,$index');
  ...
  /// Send `interested` or `not interested` to the other party to indicate whether you are interested in the resource it has
  ///
  ...
  /// The Peer is dispose.
  ///
  /// The disposed peer will no longer be able to send or monitor data, the state data will also be restored to the initial state, and the previously added event monitoring
  /// will be removed.
...
class TCPConnectException implements Exception {
  final Object e;
  TCPConnectException(this.e);
}

class _TCPPeer extends Peer {
  Socket? _socket;
  _TCPPeer(String? localPeerId, CompactAddress? address,
      List<int>? infoHashBuffer, int piecesNum, this._socket,
      {bool enableExtend = true, bool enableFast = true})
...
class _UTPPeer extends Peer {
  ...
  UTPSocket? _socket;
  _UTPPeer(String? localPeerId, CompactAddress? address,
      List<int>? infoHashBuffer, int piecesNum, this._socket,
      {bool enableExtend = true, bool enableFast = true})
      : super(localPeerId, address, infoHashBuffer, piecesNum,
            type: PeerType.UTP,
