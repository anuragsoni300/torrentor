...
class UTPSocketClient extends _UTPCloseHandler with _UTPSocketRecorder {
  ...
  Future close([dynamic reason]) async {
    ...
    indexMap.forEach((key, socket) {
      dynamic r = socket.close();
      if (r != null) f.add(r);
    });
    clean();

    _connectingSocketMap.forEach((key, Completer<UTPSocket>? c) {
...
class _UTPSocket extends UTPSocket {
  ...
  void _requestSendData([List<int>? data]) {
    ...
    var window = min(_allowWindowSize as int, remoteWndSize as int);
  ...
  Future close([dynamic reason]) {
    if (isClosed) return Future.delayed(const Duration(milliseconds: 0));
    connectionState = _UTPConnectState.CLOSING;
    _closeCompleter = Completer();
  ...
  void _caculateRTO(UTPPacket packet) {
    var packetRtt =
        getNowTimestamp(_startTimeOffset) - packet.sendTime!.toInt();
    if (_srtt == null) {
      _srtt = packetRtt.toDouble();
      _rttvar = packetRtt / 2;
    } else {
      _rttvar =
          (1 - 0.25) * _rttvar!.toDouble() + 0.25 * (_srtt! - packetRtt).abs();
      _srtt = (1 - 0.125) * _srtt!.toDouble() + 0.125 * packetRtt;
    }
  ...
  UTPPacket? _ackPacket(int seq) {
    ...
    if (packet != null) {
      ...
      var rtt = now - packet.sendTime!.toInt();
      // 重发的packet不算
  ...
  void _ledbatControl(int ackedSize, int delay) {
    ...
    var ourdelay = min(minPacketRTT as int,
        currentdelay); // delay会影响增加窗口的大小，这种获得的delay增加窗口会很激进
    // var ourdelay = currentdelay; // 这种方法会温和一些
    ...
    _allowWindowSize = _allowWindowSize! + scaledGain.toInt();
  ...
  /// request to send ACK to the remote
  ///
  /// The request is pushed into the event queue，If there is a new one before the trigger ACK to issue， And the ACK is not less than the one to be sent ACK，
  /// will cancel the send，Replace with the latest ACK
  void requestSendAck() {
    if (isClosed) return;
    if (!isConnected && !isClosing) return;
    ...
    _requestSendAckMap[ack!] = Timer(Duration.zero, () {
      ...
      if (!sendPacket(packet, 0, false, false) &&
        ...
        // Send failure unless it is the last FIN, there is no need to continue to re-send
  ...
  void addReceivePacket(UTPPacket packet) {
    ...
    if (isClosing) {
      if (lastRemoteSeq == _finalRemoteFINSeq) {
        ...
        //FIN countdown is reset every time new data is received
  ...
  ///
  /// [times] It's the first time it's reposted
  ///
  /// [increase] Indicates whether to increment seq or not, if type is ST_STATE, the value will not increment seq regardless of whether it is true or false
  ///
  ...
  bool sendPacket(UTPPacket packet,
    ...
    if (packet.type == ST_DATA ||
        packet.type == ST_SYN ||
        packet.type == ST_FIN) {
      lastAck = lastRemoteSeq; // The DATA type is sent with the latest ack
    }
    if (packet.type == ST_DATA || packet.type == ST_STATE) {
      // Carry the latest selectiveAck
...
void _processFINMessage(_UTPSocket? socket, UTPPacket packetData) async {
  ...
  socket.remoteAcked(
      packetData.ack_nr!, packetData.timestampDifference!, false);
}
...
void _processDataMessage(_UTPSocket? socket, UTPPacket packetData,
  ...
  if (socket.isConnected || socket.isClosing) {
    ...
    socket.lastRemotePktTimestamp = packetData.sendTime;
    socket.addReceivePacket(packetData);
    socket.remoteAcked(packetData.ack_nr!, packetData.timestampDifference!,
        false, selectiveAcks);
